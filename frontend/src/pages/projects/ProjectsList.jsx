...
export default function ProjectsList() {
  ...
  const handleViewProject = (e, projectId) => {
    ...
    navigate(`/app/projects/${projectId}/overview`, { state: { from: 'all' } });
  };
  ...
  const handleCancelProject = async (e, project) => {
    e.preventDefault(); e.stopPropagation();
    if (window.confirm(`${t('msg.confirm_delete')} "${project.project_name}"?`)) {
      try {
        await projectsAPI.update(project.id, { ...project, status: "CANCELLED" });
        fetchData();
  ...
  const getStatusBadge = (status) => {
    ...
    return <span className={`px-3 py-1 text-xs rounded-full font-medium ${badge.bg} ${badge.text}`}>{badge.label}</span>;
  };
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      {canWrite && project.status !== "CANCELLED" && project.status !== "COMPLETED" && (
                        <button onClick={(e) => handleCancelProject(e, project)}
                          className="text-red-600 hover:text-red-900 p-1 hover:bg-red-50 rounded transition" title={t('cancel_project')}>
